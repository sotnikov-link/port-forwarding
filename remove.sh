...
# Скрипт для удаления правил в iptables, чтобы не перенаправлять запросы
...
# ./port-forwarding/remove.sh # для удаления правил
...
if [ -z "$FROM_IP" ] && [ -z "$FROM_PORT" ] && [ -z "$FROM_IF" ] &&
  ...
  echo './port-forwarding/remove.sh'
  echo ''
...
# Правила
iptables -t nat -D PREROUTING -d $EXTERNAL_IP -p tcp -m tcp --dport $EXTERNAL_PORT -j DNAT --to-destination $INTERNAL_TARGET_IP:$INTERNAL_TARGET_PORT
iptables -t nat -D POSTROUTING -d $INTERNAL_TARGET_IP -p tcp -m tcp --dport $INTERNAL_TARGET_PORT -j SNAT --to-source $INTERNAL_OWN_IP
iptables -t nat -D OUTPUT -d $EXTERNAL_IP -p tcp -m tcp --dport $INTERNAL_TARGET_PORT -j DNAT --to-destination $INTERNAL_TARGET_IP
iptables -D FORWARD -i $EXTERNAL_IF -d $INTERNAL_TARGET_IP -p tcp -m tcp --dport $INTERNAL_TARGET_PORT -j ACCEPT
